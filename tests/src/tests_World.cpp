...
TEST_CASE("[World] Basic tests", "[Physics][World]")
{
  ...
  auto transform2 = BodyTransform{{.position = {2.0f, 1.0f, 0.0f}}};
  ...
  {
    world.addGlobalConstantForce({{0.0f, -1.0f, 0.0f}, Force::Type::Force});
    world.step(0.5);
    CHECK(transform1.getPosition() == glm::vec3{0.0f, -0.5f, 0.0f});
    CHECK(transform2.getPosition() == glm::vec3{2.0f, 0.5f, 0.0f});
    world.step(1.0);
    CHECK(transform1.getPosition() == glm::vec3{0.0f, -2.0f, 0.0f});
    CHECK(transform2.getPosition() == glm::vec3{2.0f, -1.0f, 0.0f});
  }

  SECTION("Apply body-specific constant forces")
  {
    body2->addConstantForce({{0.0f, -1.0f, 0.0f}, Force::Type::Force});
    world.step(0.5);
  ...
  {
    body2->addForce({{0.0f, -1.0f, 0.0f}, Force::Type::Force});
    world.step(0.5);
    CHECK(transform1.getPosition() == glm::vec3{0.0f, 0.0f, 0.0f});
    CHECK(transform2.getPosition() == glm::vec3{2.0f, 0.5f, 0.0f});
    world.step(0.5);
    CHECK(transform1.getPosition() == glm::vec3{0.0f, 0.0f, 0.0f});
    CHECK(transform2.getPosition() == glm::vec3{2.0f, 0.0f, 0.0f});
    body2->addForce({{0.0f, -1.0f, 0.0f}, Force::Type::Force});
    world.step(1.0);
    CHECK(transform1.getPosition() == glm::vec3{0.0f, 0.0f, 0.0f});
    CHECK(transform2.getPosition() == glm::vec3{2.0f, -1.5f, 0.0f});
  }
}
